...
import kevin.app.zhiliaodaily.activity.LatestContentActivity;
import kevin.app.zhiliaodaily.activity.MainActivity;
import kevin.app.zhiliaodaily.adapter.MainNewsItemAdapter;
import kevin.app.zhiliaodaily.model.Before;
...
import kevin.app.zhiliaodaily.view.Kanner;
...
public class MainFragment extends BaseFragment {
    ...
    protected View initView(final LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        kanner.setOnItemClickListener(new Kanner.OnItemClickListener() {
            ...
            public void click(View v, Latest.TopStoriesEnity enity) {
                ...
                storiesEntity.setId(enity.getId());
                storiesEntity.setTitle(enity.getTitle());
            ...
            HttpUtils.get(Constant.LATESTNEWS, new TextHttpResponseHandler() {
                ...
                public void onSuccess(int i, Header[] headers, String responseString) {
                    ...
                    db.execSQL("replace into CacheList(date,json)values("
                            + Constant.LATEST_COLUMN + ",'" + responseString + "')");
                    db.close();
    ...
    private void parseLatestJson(String responseString){
        ...
        date=latest.getDate();
        kanner.setTopEnities(latest.getTop_stories());
    ...
    private void loadMore(final String url){
        ...
        if (HttpUtils.isNetworkConnected(mActivity)){
            ...
            if (cursor.moveToFirst()){
                ...
                sb.getView().setBackgroundColor(getResources().getColor(((MainActivity)mActivity).isLight()?
                android.R.color.holo_blue_dark:android.R.color.black));
                sb.show();
            }
            cursor.close();
            db.close();
    ...
    private void parseBeforeJson(String responeseString){
        Gson gson=new Gson();
        before = gson.fromJson(responeseString, Before.class);
        if (before==null){
            isLoading=false;
            return;
        }
